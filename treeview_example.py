...
class FileListingCellDataExample:
    ...
    def file_pixbuf(self, column, cell, model, iter, data):
        filename = os.path.join(self.dirname, model.get_value(iter, 0))
        filestat = os.stat(filename)
        if stat.S_ISDIR(filestat.st_mode):
            pb = folderpb
    ...
    def file_name(self, column, cell, model, iter, data):
        cell.set_property('text', model.get_value(iter, 0))
        return

    def file_size(self, column, cell, model, iter, data):
        filename = os.path.join(self.dirname, model.get_value(iter, 0))
        filestat = os.stat(filename)
        cell.set_property('text', str(filestat.st_size))
        return

    def file_mode(self, column, cell, model, iter, data):
        filename = os.path.join(self.dirname, model.get_value(iter, 0))
        filestat = os.stat(filename)
    ...
    def file_last_changed(self, column, cell, model, iter, data):
        filename = os.path.join(self.dirname, model.get_value(iter, 0))
        filestat = os.stat(filename)
